...
const i18n = {
    en: {
        home:'Home', game:'Game', themeDark:'Dark', themeLight:'Light',
    ...
    zh: {
        home:'主页', game:'游戏', themeDark:'暗黑', themeLight:'明亮',
...
const G = {};
[
    'nav-home-text','nav-game','btn-lang-text','btn-theme-text',
    ...
    'btn-confirm-reset','btn-cancel-reset','btn-cb',
...
const getColors = () => document.documentElement.getAttribute('data-colorblind') === 'true' ? COLORS_CB : COLORS_N;

/* ─────────────────────────────────────────────────────────────────
   CANVAS RESIZE
   The canvas internal resolution must exactly match its CSS display
   size so that hit-testing coordinates are correct.
   ───────────────────────────────────────────────────────────────── */
function resizeCanvas() {
    const wrapper = G['canvas-wrapper'];
    const w = wrapper.clientWidth;
    const ratio = window.innerWidth >= 768 ? 2.4 : 1.9;
    const h = Math.round(w / ratio);
    /* Set the canvas drawing buffer to match CSS pixels exactly */
    canvas.width  = w;
    canvas.height = h;
    /* Also set explicit CSS height so wrapper matches */
...
class Circle {
    ...
    draw(t) {
        ...
        const fs = Math.max(8, Math.min(pr * 0.35, 14));
...
function drawBackground() {
    ...
    ctx.strokeStyle = 'rgba(80,100,255,0.05)'; ctx.lineWidth = 1;
...
/* ─────────────────────────────────────────────────────────────────
   INPUT HANDLING — THE CRITICAL FIX
   
   We must convert pointer coordinates from page/viewport space
   into canvas buffer space correctly.
   
   canvas.getBoundingClientRect() gives us the CSS-pixel rect.
   canvas.width / canvas.height   gives us buffer pixels.
   scaleX = bufferWidth  / cssWidth
   scaleY = bufferHeight / cssHeight
   
   Since we set width=clientWidth and height=clientHeight in
   resizeCanvas(), scale should always be 1.0 — but we
   calculate it explicitly for safety.
   ───────────────────────────────────────────────────────────────── */
function handleInput(clientX, clientY) {
    ...
    /* Find the topmost circle that was hit */
    let hit = null;
    for (let i = circles.length - 1; i >= 0; i--) {
        const c = circles[i];
        const dist = Math.sqrt((cx - c.x) * (cx - c.x) + (cy - c.y) * (cy - c.y));
        if (dist <= c.r) { hit = c; break; }
    }

    if (!hit) return;  /* Missed all circles — no penalty */

    /* Popup position in percent of wrapper */
    ...
    if (hit === state.targetCircle) {
        ...
        if (state.score >= state.level * 50) {
            state.level++;
            playLevelUpSound();
            spawnParticles(canvas.width / 2, canvas.height / 2, '#f1c40f', 50);
        }
        updateHUD();
        generateLevel();
    } else {
        ...
        if (state.lives <= 0) {
            spawnParticles(hit.x, hit.y, '#e74c3c', 40);
            endGame();
        } else {
            generateLevel();
        }
...
function updateLang() {
    ...
    G['btn-theme-text'].textContent  = dark ? t.themeLight : t.themeDark;
...
function toggleColorBlind() {
    const root = document.documentElement;
    const isOn = root.getAttribute('data-colorblind') === 'true';
    root.setAttribute('data-colorblind', String(!isOn));
    G['btn-cb'].setAttribute('aria-pressed', String(!isOn));
}
...
/* ─────────────────────────────────────────────────────────────────
   INIT — Attaches ALL event listeners in one place
   ───────────────────────────────────────────────────────────────── */
function init() {
    resizeCanvas();
    updateLang();
    ...
    /* Canvas click — pointerdown for lowest latency */
    canvas.addEventListener('pointerdown', function(e) {
        e.preventDefault();
        handleInput(e.clientX, e.clientY);
    }, { passive: false });

    /* Also support mouse click as fallback */
