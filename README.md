# EBU5315_G26

## Project Title:
GeoCircle Interactive

## Members:
Dan Li, 2024213604, 241118256, jp2024213604@qmul.ac.uk
Ruohan Qi, 2024213576, 241118360, jp2024213576@qmul.ac.uk
Yue Liu, 2024213627, 241118326, jp2024213627@qmul.ac.uk

## Assigned TA:
Wenjing Yu

## The allocation of tasks among members

| Member      | Responsible Module | Technology Stack                  |
|-------------|--------------------|-----------------------------------|
| Dan Li      | Quiz Module        | Native HTML/CSS/JavaScript        |
| Ruohan Qi   | Homepage Module    | Native HTML/CSS/JavaScript        |
| Yue Liu     | Game Module        | Native HTML/CSS/JavaScript + Canvas |
